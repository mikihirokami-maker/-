...
def save_json(file_path, data):
    ...
    for item in data:
        ...
        if 'image_file' in item_copy:
            del item_copy['image_file'] # 一時的なファイルオブジェクトは保存しない
        # base64画像データがある場合は保存しない（重くなるため、今回はメモリ内処理前提）
        # ※本来はファイルパス保存が望ましいが、セッション維持のため簡易化
...
def upload_image_to_imgur(image_data):
    ...
    try:
        # image_data は bytes または file-like object
        if hasattr(image_data, 'getvalue'):
            payload = {"image": image_data.getvalue()}
        else:
...
def post_to_threads(account, text, image_obj=None):
    ...
    try:
        ...
        creation_id = res['id']
        time.sleep(10 if image_url else 2)
...
def paste_image_component():
    # JavaScriptでクリップボードの画像を読み取り、st.text_inputに入れる裏技
    # ※Streamlitの標準機能でクリップボード画像を受け取るのは難しいため、
    # ユーザーには「ファイル選択」または「ドラッグ＆ドロップ」を推奨しつつ、
    # ペースト機能は「st.file_uploader」が現在対応している範囲（ブラウザ依存）に任せるのが安全です。
    # しかし要望通り「コピーした画像を貼れる」ようにするため、PasteButton的な挙動を模倣します。
    pass 
    # Streamlitのfile_uploaderは、フォーカスが当たっている状態でCtrl+Vを押すと
    # ファイルとして貼り付けられるブラウザが多いです。
    # そのため、特別なJSを使わずとも file_uploader で要件を満たせる可能性があります。
...
with tab2:
    ...
    if not st.session_state.accounts:
        ...
        with st.container():
            ...
            # 【ポイント】st.file_uploader は多くのブラウザで「フォーカス時にCtrl+V」でファイル化して貼り付け可能です
            # 明示的に「ここに画像をペースト」と書くことでユーザーを誘導します
            img_file = c1.file_uploader("画像 (ドラッグ＆ドロップ、または枠内クリック後にCtrl+V)", type=['png','jpg','jpeg','webp'], key="form_file")
